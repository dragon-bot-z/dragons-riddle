...
contract DragonsRiddle {
    ...
    function submitGuess(string calldata guess) external payable {
        ...
        entries[currentRiddleId].push(Entry({player: msg.sender, guess: guess, timestamp: block.timestamp}));
    ...
    function getStats()
        external
        view
        returns (uint256 _totalRiddles, uint256 _totalSolved, uint256 _totalPrizesPaid, uint256 _currentPot)
    {
